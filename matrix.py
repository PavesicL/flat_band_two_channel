...
def generate_hopping_matrix(subspace : str, full_basis : list[STATE], index_list : list[int], n : int, p : params) -> np.array:
	...
	for i, si in enumerate(full_basis):
		for j, sj in enumerate(full_basis):
			...
			val = general_hopping_matrix[i_ind][j_ind](mL=si.mL, mR=si.mR, nL=sj.mL, nR=sj.mR, vL=p.v_L, vR=p.v_R, tsc=p.tsc, tspinL=p.tspin_L, tspinR=p.tspin_R, l=p.LL, Ex=p.Ex_imp, Ey=p.Ey_imp)
			...
			if p.add_periodic_hopping_blocks:
				...
				if si.dM == smallest_dM and sj.dM == largest_dM:
					...
					val += general_hopping_matrix[i_ind][j_ind](mL=si.mL, mR=si.mR, nL=si.mL-1, nR=si.mR, vL=p.v_L, vR=p.v_R, tsc=p.tsc, tspinL=p.tspin_L, tspinR=p.tspin_R, l=p.LL, Ex=p.Ex_imp, Ey=p.Ey_imp)
					val += general_hopping_matrix[i_ind][j_ind](mL=si.mL, mR=si.mR, nL=si.mL, nR=si.mR+1, vL=p.v_L, vR=p.v_R, tsc=p.tsc, tspinL=p.tspin_L, tspinR=p.tspin_R, l=p.LL, Ex=p.Ex_imp, Ey=p.Ey_imp)
